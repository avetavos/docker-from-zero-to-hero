...
|---|---|---|
| เขียน `.dockerignore` ครอบคลุมทุก path ที่ไม่จำเป็น | build context เล็กและส่งเร็ว ลดความเสี่ยง secret รั่วไหลเข้า image | ต้องดูแลรายการให้ตรงกับโครงสร้างโปรเจกต์ที่เปลี่ยนไปเรื่อยๆ |
| เลือก base image ที่เล็กกว่า (alpine แทน full distro) | ลดขนาด final image ได้หลักร้อย MB | native addon บางตัวที่ compile กับ `glibc` อาจต้อง build ใหม่สำหรับ `musl` |
